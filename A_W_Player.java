...
public class A_W_Player extends Player
{
    ...
    public void makeMove(Board theBoard) {
        int[] bestMove = minimax(theBoard, symbol, symbol, Integer.MIN_VALUE, Integer.MAX_VALUE);
        int col = bestMove[0];
        int row = bestMove[1];
 ...
 */
     ...
     * recursion. 
     */
    private int[] minimax(Board board, int playerSymbol, int maximizerSymbol, int alpha, int beta) {
        int[] bestMove = new int[]{-1, -1, 0};
        int bestScore = (playerSymbol == Board.X) ? Integer.MIN_VALUE : Integer.MAX_VALUE;
        int opponentSymbol = (maximizerSymbol == Board.X) ? Board.O : Board.X;
        ...
        for (int col = 0; col < board.getSize(); col++) {
            for (int row = 0; row < board.getSize(); row++) {
                if (board.isOpen(col, row)) {
                    ...
                    int score;
                    score = minimax(possibleBoard, (playerSymbol == maximizerSymbol) ? opponentSymbol : maximizerSymbol, maximizerSymbol, alpha, beta)[2];
